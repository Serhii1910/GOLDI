...
(function(window) {
	...
	function View(p) {
		...
		$('#OR-symbol').on('keyup.base', function(e) {
			var input = $('#OR-symbol');
			feedbackTarget = input.parentsUntil('li');
		...
		$('#AND-symbol').on('keyup.base', function(e) {
			var input = $('#AND-symbol');
			feedbackTarget = input.parentsUntil('li');
		...
		$('#NOT-symbol').on('keyup.base', function(e) {
			var input = $('#NOT-symbol');
			feedbackTarget = input.parentsUntil('li');
	...
	Handlebars.registerHelper('notMaximum', function(){
		var maxnumber = machine.MaxOutputNumber;
	...
	function blankEditor() {
		var source = $('#editor-blank-template').html();
	...
	function linkEditor(data) {
		...
		var source = $('#transitiondescr-template').html();
		var template = Handlebars.compile(source);
		var html = template(data);
		editor.innerHTML = html;
		MathJax.Hub.Queue(['Typeset', MathJax.Hub, '#editor']);
	...
	function editStateName(e) {
		...
		var pseudoTarget = $(e.target).closest('tr');
		var input = pseudoTarget.find('input');
		var output = pseudoTarget.find('.output');
	...
	function editStateOutput(e) {
		...
		var input = pseudoTarget.find('input');
		var output = pseudoTarget.find('.output');
		var idx = pseudoTarget.attr('id').replace(/stateoutput/, '');
	...
	function addStateOutput(e) {
		var aso = $('#add-state-output');
		var input = aso.find('input');
		var button = aso.find('span');
		var idx = $('.output').length -1;
		var pseudoTarget = $(e.target).closest('tr');
		feedbackTarget = pseudoTarget;

		if(machine.OutputNumber < machine.MaxOutputNumber && machine.OutputNumber <= idx){
	...
	function editInputValue(e) {
		...
		/*
		output.addClass('hidden');
		input.val(selected.input).removeClass('hidden').focus();
		input.on('blur', function() {
			replaceInput(input, output, false);
		});
		*/
