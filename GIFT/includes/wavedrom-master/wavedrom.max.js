...
var JsonML;
//var content = []; in case of onclick with modified contetn array //By LennartP
...
function renderWaveLane (root, content, index) {
    var i, j, k, g, gg, title, b, b2 , labels = [1], name,
        ...
        if (name) { // check name
            ...
            gg = parse([
                'g',
                {
                    ...
                    transform: 'translate(' + (xoffset * lane.xs) + 20 + ', 0)'
                }
            ]);
            ...
            if (content[j][1]) {
                for (i = 0; i < content[j][1].length; i += 1) {
                    ...
                    b.setAttribute('transform', 'translate(' + (i * lane.xs) + ')');
                    b.setAttribute('onclick', 'Simulator.click(' + i + ', ' + j + ', ' + index + ')');
                    b.setAttribute('stroke', 'black');
                    b.setAttribute('stroke-width',"1");

                    b2 = document.createElementNS(svgns, 'rect');
                    b2.setAttribute('width', "21");
                    b2.setAttribute('height', "25");
                    b2.setAttribute('transform', 'translate(' + (i * lane.xs) + ',-2.5)');
                    //b2.setAttribute('style', "fill:rgb(0,0,255)");
                    b2.style.opacity = "0";
                    b2.setAttribute('onclick', 'Simulator.click(' + i + ', ' + j + ', ' + index + ')');

                    gg.insertBefore(b, null);
                    gg.insertBefore(b2, b);
                }
...
function renderAssign (index, source) {
    ...
    svgcontent.setAttribute('viewBox', '0 0 ' + 1.2*width + ' ' + height);
...
function renderWaveForm (index, source, output) {
    ...
    if (source.signal) {
        ...
        svgcontent.setAttribute('width', 1.2*width);
        svgcontent.setAttribute('height', 1.2*height);
        svgcontent.setAttribute('overflow', 'hidden');
        //root.setAttribute('transform', 'translate(' + (lane.xg + 0.5) + ', ' + ((lane.yh0 + lane.yh1) + 0.5) + ')');
        root.setAttribute('transform', 'translate(' + (30.5) + ', ' + ((lane.yh0 + lane.yh1) + 0.5) + ')');
    } else if (source.assign) {
