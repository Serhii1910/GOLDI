class Grid {
    constructor(numInputs, numColumns, numOutputs, fixedAnd, fixedOr) {
        this.lineDistance = 25;
        this.sectionBorder = this.lineDistance / 2;
        this.inputLineDist = 10;
        this.inputLines = [];
        this.inputNodes = [];
        this.verticalLines = [];
        this.outputLines = [];
    ...
    setParams(inputs, outputs, columns = this.columns) {
        this.inputs = inputs;
    ...
    drawNodes(svg, lines, fixed, nodeCbk, x = 0, y = 0) {
        const nodes = [];
    ...
    drawLines(svg, lines, posFct, valueFct = null) {
    ...
    drawSection(svg, x, y, x2, y2) {
        const ele = svg.drawRect(x, y, x2 - x, y2 - y);
        ele.css({
            fill: '#ffffff',
    ...
    drawInputTriangle(svg, xOffset, yOffset) {
        const triWidth = 1.5;
...
class GridNode {
    constructor(ui, state, styleCbk, fctCbk) {
        this.ui = ui;
...
class GAL extends Grid {
    constructor(inputs, outputs, linksPerOutput) {
    ...
    setParams(inputs, outputs, linksPerOutput) {
        this.linksPerOutput = linksPerOutput;
