...
// === OLD CODE ===
// This file is decommissioned and included only for reference
...
var simcir = function ($) {
    ...
    var createDeviceController = function (device) {
        ...
        var layoutUI = function () {
            ...
            device.$ui.children('.simcir-device-label').attr({x: w / 2, y: h + fontSize});
        };
        ...
        var createUI = function () {
            ...
            var label_dblClickHandler = function (event) {
                ...
                var $labelEditor = $('<input type="text"/>').addClass('simcir-label-editor').val($label.text()).on('keydown', function (event) {
                    if (event.keyCode == 13) {
                        // ENTER
                        setLabel($(this).val());
                        $dlg.remove();
    ...
    var showDialog = function (title, $content) {
        ...
        var $dlg = $('<div></div>').addClass('simcir-dialog').css({position: 'absolute'}).append($title.css('float', 'left')).append($closeButton.css('float', 'right')).append($('<br/>').css('clear', 'both')).append($content);
        $('BODY').append($dlg);
    ...
    var createScrollbar = function () {
        ...
        var layout = function () {
            ...
            calc(function (unitSize) {
                $bar.children('rect').attr({x: 0, y: 0, width: _width, height: _barSize * unitSize});
    ...
    var createWorkspace = function (data) {
        ...
        var $workspace = createSVG(
            workspaceWidth, workspaceHeight).attr('class', 'simcir-workspace');
        ...
        !function () {
            ...
            $defs.append(createSVGElement('pattern').attr({
                ...
                createSVGElement('rect').attr('class', 'simcir-pin-hole').attr({x: 0, y: 0, width: 1, height: 1})));
        ...
        var beginConnect = function (event, $target) {
            ...
            var insertJunction = function (event) {
                ...
                let dev = createDevice(junction, false, scope);
                let size = controller(dev).getSize();
                transform(dev, x - toolboxWidth, y - size.height / 2);
                adjustDevice(dev);
                addDevice(dev);
                if ($srcNode.attr('simcir-node-type') == 'out') {
                    connect($srcNode, controller(dev).getInputs()[0].$ui);
                    $srcNode = controller(dev).getOutputs()[0].$ui;
                }
                else {
                    connect($srcNode, controller(dev).getOutputs()[0].$ui);
                    $srcNode = controller(dev).getInputs()[0].$ui;
                }
                updateConnectors();
    ...
    var setupSimcir = function ($placeHolder, data) {
        var $workspace = simcir.createWorkspace(data).ui;
