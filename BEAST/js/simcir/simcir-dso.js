...
!function($, $s) {
  ...
  var createDSOFactory = function() {
    ...
    var createPanel = function() {
      ...
      var $lcd = SVGGraphics.createSVGElement('path').
        css('stroke', 'none').css('fill', '#ffcc00');
      var setLCDText = function(text) {
        $lcd.attr('d', createFontPath(text, 4, 4, 1) );
      };
      var $lcdPanel = SVGGraphics.createSVGElement('g').
        append(SVGGraphics.createSVGElement('rect').
          css('stroke', 'none').
          css('fill', '#000000').
        ...
        SVGGraphics.transform($lcdPanel, unit * 1.5, 0);

      var $playing = SVGGraphics.createSVGElement('path').
      ...
      var btnAttr = {x : 0, y : 0, width : unit, height : unit,
          rx : 1, ry : 1};
      var $btnRect = SVGGraphics.createSVGElement('rect').
        attr(btnAttr).
        css('stroke', 'none').
        css('fill', '#999999').
        css('opacity', 0);
      var $btn = SVGGraphics.createSVGElement('g').
        append($btnRect).
        append(SVGGraphics.createSVGElement('rect').
            attr(btnAttr).
            css('stroke-width', 1).
      ...
      var $panel = SVGGraphics.createSVGElement('g').
    ...
    return function(device) {
      ...
      device.createUI = function() {
        ...
        var $rect = SVGGraphics.createSVGElement('rect').
        ...
        for (var i = 0; i < device.getInputs().length; i += 1) {
          var inNode = device.getInputs()[i];
            SVGGraphics.transform(inNode.$ui, 0, unit *
        ...
        SVGGraphics.transform(panel.$ui, unit / 2,
  ...
  BeastController.registerDefaultComponent(new BasicComponent('DSO', "Digital Storage Oscilloscope", createDSOFactory()));
